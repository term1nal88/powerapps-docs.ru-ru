...
Если приложение использует пользовательскую сущность, то перед использованием приложения необходимо явно предоставить ему привилегии в роли безопасности.  Можно добавить эти привилегии в существующую роль безопасности или создать пользовательскую роль безопасности. Существует ряд минимальных привилегий, которые необходимы для использования новой роли, — см. раздел [Минимальные привилегии для запуска приложения](#minimum-privileges-to-run-app).

> [!TIP]
> Если вы хотите создать настраиваемую роль безопасности с минимальными привилегиями для запуска приложения, обратитесь к разделу [Минимальные привилегии для запуска приложения](#minimum-privileges-to-run-app) ниже.
...
## <a name="minimum-privileges-to-run-app"></a>Минимальные привилегии для запуска приложения
При создании пользовательской роли безопасности необходимо включить в роль безопасности минимальные привилегии, чтобы пользователь мог запустить приложение. Мы создали решение, которое предоставляет роль безопасности с минимальными привилегиями. Вы можете импортировать это решение.  

Для начала скачайте следующее решение из Центра загрузки: [Роль безопасности с минимальными привилегиями для CDS for Apps](http://download.microsoft.com/download/6/5/5/6552A30E-05F4-45F0-AEE3-9BB01E13118A/MinprivilegeSecRole_1_0_0_0.zip).

Затем выполните необходимые действия для импорта решения. Они приведены в разделе [Импорт, обновление и экспорт решений](../maker/common-data-service/import-update-export-solutions.md).

При импорте решения создается роль **min prv apps use**, которую можно скопировать (см. раздел [Создание роли безопасности](https://docs.microsoft.com/en-us/dynamics365/customer-engagement/admin/create-edit-security-role#create-a-security-role-by-copy-role)). После завершения копирования роли перейдите на каждую вкладку ("Базовые записи", "Управление бизнес-процессами", "Настройка" и т. д.) и установите соответствующие привилегии. 

> [!IMPORTANT]
> Перед импортом решения в рабочую среду протестируйте его в среде разработки. 
