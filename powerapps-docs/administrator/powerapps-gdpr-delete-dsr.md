...
\*** Для сред, которые содержат базу данных Common Data Service (CDS) для приложений, разрешения среды (то есть, каким пользователям назначены роли создателя и администратора в среде) хранятся в виде записей в этой базе данных. Руководство по реагированию на запросы DSR для пользователей CDS для приложений см. в статье [Реагирование на запросы по правам субъекта данных (DSR) на получение данных клиента службы Common Data Service для приложений](common-data-service-gdpr-dsr-guide.md).
...
\** С выходом службы CDS для приложений, если в среде создается база данных, разрешения среды и приложения на основе модели хранятся в виде записей в экземпляре этой базы данных. Руководство по реагированию на запросы DSR для пользователей CDS для приложений см. в статье [Реагирование на запросы по правам субъекта данных (DSR) на получение данных клиента службы Common Data Service для приложений](common-data-service-gdpr-dsr-guide.md).
...
> [!IMPORTANT]
> Эта функция работает только в средах, в которых отсутствует экземпляр базы данных в CDS для приложений.
...
С выходом службы CDS для приложений, если в среде создается база данных, эти назначения ролей хранятся в виде записей в экземпляре базы данных.
...
Администраторы могут удалять все назначения ролей среды для пользователя во всех средах без базы данных CDS для приложений с помощью одного из командлетов [PowerShell для администратора PowerApps](https://go.microsoft.com/fwlink/?linkid=871804) — **Remove-AdminEnvironmentRoleAssignment**:
...
> [!IMPORTANT]
> Эта функция работает только в средах, в которых отсутствует экземпляр базы данных CDS для приложений.

### <a name="for-environments-with-a-cds-for-apps-database"></a>Для сред с базой данных CDS для приложений
С выходом службы CDS для приложений, если в среде создается база данных, эти назначения ролей хранятся в виде записей в экземпляре базы данных. Ознакомьтесь с документацией о том, как удалить персональные данные из экземпляра базы данных в CDS для приложений.
...
## <a name="step-10-delete-the-users-personal-data-in-instances-of-cds-for-apps"></a>Шаг 10. Удаление персональных данных пользователя в экземплярах CDS для приложений
Некоторые лицензии PowerApps, включая "План сообщества PowerApps", дают пользователям организации возможность создавать экземпляры CDS для приложений и приложения в этой службе. "План сообщества PowerApps" — это бесплатная лицензия, которая позволяет пользователям испытать CDS для приложений в отдельной среде. Сведения о возможностях, включенных в каждую лицензию PowerApps, см. на странице с ценами на PowerApps.

Руководство по реагированию на запросы DSR для пользователей CDS для приложений см. в статье [Реагирование на запросы по правам субъекта данных (DSR) на получение данных клиента службы Common Data Service для приложений](common-data-service-gdpr-dsr-guide.md).
