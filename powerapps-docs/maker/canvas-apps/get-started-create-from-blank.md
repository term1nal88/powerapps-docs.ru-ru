...
1. В разделе **Источник данных** щелкните стрелку вниз, чтобы открыть список источников данных для приложения, и выберите **Schedule**.

    ![Выбор источника данных](./media/get-started-create-from-blank/select-schedule.png)

1. В разделе **Макет** щелкните направленную стрелку вниз, чтобы открыть список макетов, и выберите **Заголовок, подзаголовок и текст**.

    ![Выбор макета](./media/get-started-create-from-blank/select-layout.png)

1. В разделе **Заголовок2** измените заголовок столбца **Backup** на **Volunteer**.
...
4. Выберите коллекцию и убедитесь, что в списке отображается свойство **[Items](controls/properties-core.md)**.
...
1. Измените значение свойства **Items**, скопировав в строку формул следующую формулу:
...
5. Введите букву **i** в поле поиска, затем коснитесь или щелкните кнопку сортировки два раза (или любое четное число раз).
...
1. Очистите весь текст в поле поиска.

6. В верхней части экрана выберите элемент управления **[Метка](controls/control-text-box.md)**, а затем замените текст **[Title]** строкой **Просмотр записей**.
