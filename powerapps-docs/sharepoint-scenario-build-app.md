...
**Подсказка.** [Пакет загрузки](https://aka.ms/o4ia0f) для нашего примера содержит полную версию этого приложения: project-details-app.msapp.
...
4. Вставьте еще одну кнопку со следующими свойствами:
   ...
   * свойство **Text** = **"Обновить сведения"**.
     ...
     **Примечание.** Кнопка называется **Update Details** (Обновить сведения), но сначала мы перейдем на экран **ViewProjects** (Просмотр проектов), чтобы выбрать проект для обновления.
...
**Примечание.** Мы подумали, что будет здорово, если вы создадите такой экран, хотя позже мы также добавим в приложение страницу, которая позволяет изменять все поля проекта (включая поле "Руководитель").
